...
public class AIUtil {
    ...
    public static int outAI(List<Integer> input, List<Integer> guiCard, boolean extHu) {
        ...
        double maxSpecial = Double.MIN_VALUE;
        ...
        for (Integer c : input) {
            if (!processed.contains(c)) {
                ...
                if (!guiCard.contains(c)) {
                    ...
                    // score大于4听牌
                    System.out.println("out: " + MaJiangDef.cardToString(c) + " score: " + score + " isSpecial: " + isSpecial);
                    if (isSpecial > maxSpecial) {
                        maxSpecial = isSpecial;
                    }
                    if (isSpecial >= max) {
                        max = isSpecial;
                    }
        ...
        for (Map.Entry<Integer, Double> integerDoubleEntry : map.entrySet()) {
            if (integerDoubleEntry.getValue() == maxSpecial) {
